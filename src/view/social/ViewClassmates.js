...
import {most_popular_in_class, list_same_classes} from "../../api/MostPopularInList";
...
class ViewClassmates extends Component {

    constructor(props) {
        super(props);
        this.state = {
            look_up_id: props.course_id,
            recommendation_ids: [],
    ...
    componentWillMount() {
        ...
        lookup_enrollment_by_id(this.state.look_up_id, (err, this_class) => {
            ...
            course_list.forEach((course_id) => {
                lookup_enrollment_by_id(course_id, (err, enrollment_obj) => {
                    ...
                    if (aggregated_ids.length === course_list.length) {
                        var query = {
                            class: Object.keys(this_class.enrolled_users),
                        ...
                        most_popular_in_class(query, (err, data) => {
    ...
    render() {
        return (
            <div align={"center"}>
                <PageTitle title="View Classmates"/>
                <table>
                    <tbody>
                    {this.state.recommendation_ids.map((user_id, index) => {
                        ...
                        else return (<tr key={"view-classmates-" + index}>
                            <td></td>
                        </tr>)
                    })
                    }
